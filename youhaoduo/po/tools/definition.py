...
class BlurDetection:
    def __init__(self, picture_list):
        ...
        for i in picture_list:
            self.strDir = i.rsplit('/', 1)[0] + '/'
            self.names.append(i.rsplit('/', 1)[-1])
    ...
    def TestSMD2(self):
        ...
        for i in range(len(imgList)):
            ...
            # result[str(imgList[i])] = score
            if 'mobile' in str(imgList[i]):
                mobile = score
            else:
                origin = score
        if mobile <= 200.0 and origin > 400.0:
            return False
        else:
            return True
    ...
    def preImgOps(self, imgName):
        ...
        sz1 = img.shape[0]
        sp = img.shape[1]
        if "dynamic_mobile" in imgName:
            cropImg = img[int(sz1*0.1):int(sz1 * 0.7), 0:sp]
        elif "definition_mobile" in imgName:
            cropImg = img[int(sz1*0.1):int(sz1 * 0.55), 0:sp]
        else:
            cropImg = img
        img2gray = cv2.cvtColor(cropImg, cv2.COLOR_BGR2GRAY)  # 将图片压缩为单通道的灰度图
        return img2gray
...
if __name__ == "__main__":
    BlurDetection = BlurDetection(picture_list=[])
    BlurDetection.TestSMD2()  # TestSMD
